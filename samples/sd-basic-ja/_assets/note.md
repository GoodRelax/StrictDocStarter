...
`01-guide-for-human.sdoc` の「添付」の節から `[LINK: DOC-NOTE]` で飛んでくる。
...
| --- | --- | --- |
| 上位要求 | REQUIREMENT | `02-upper.sdoc` の `SYS-*` |
| 下位要求 | REQUIREMENT | `03-lower.sdoc` の `SW-*` |
| テストケース | TEST_CASE | `04-tests.sdoc` の `TC-*` |
| レビュー指摘 | FINDING | `05-review.sdoc` の `RV-*` |
| 地の文 | TEXT | UID を持たない段落 |
| 章 | SECTION | `01-guide-for-human.sdoc` の入れ子 |
