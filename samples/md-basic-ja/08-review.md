...
関係で要求へ結ぶやり方もあるが、 採らない。 理由は「なぜ別ノードにしないか」にある。

レビューするのは要求だけではない。 ユースケースも同じ 3 項目を持つ
(`03-usecases.md`)。

**ユースケースを飛ばしてはならない。** ユースケースは要求より上流にあるので、
そこで見落とすと下流の全部が巻き添えになる。 見つかる欠けは 2 種類ある。

| 見つかるもの | どうなるか | どこで止めるか |
|---|---|---|
| 範囲が広すぎる | 作るものが際限なく広がる。 要求まで下りてから気づくと、 書いた要求も設計もテストも捨てることになる | `03-usecases.md` の「書き方 - Cockburn 形式」。 `UC-001` の外側を明示的に線引きしてある |
| 機能が足りない | 筋道の途中が決まっておらず、 実装の段階で誰かが勝手に決める | `UC-001` の指摘 (`Open`)。 書き出しを拒んだときに利用者へ何を知らせるかが無い |

同じ欠けが要求の側とユースケースの側の両方から見えることがある。 `UC-001` の
指摘は `SYS-003` に付いている未解決の指摘と同じ穴を指しており、 **どちらか一方を
直しても片付かない。** 上流と下流の両方に指摘が立っていることが、 その合図になる。

足したのは次の 3 項目である。 `basic.sgra` が `REQUIREMENT` と `USE_CASE` の
両方に宣言している。
...
| `Open` | 指摘あり・未対応 | 要る | 空 |
| `Fixed` | 指摘に対応した | 要る | 要る |
| `WontFix` | 検討して対応しないと決めた | 要る | 要る (理由を書く) |
...
`Rejected` という語を使わないのは、 指摘そのものを退けたように読めるためである。
...
`.md` に書いた順が食い違うと、 export が次のように止まる。 メタの塊の中でも、
組み込みの `UID` と `STATUS` の入れ替えだけは通る。 文法が宣言した独自の欄どうしを
入れ替えると、 段落の項目と同じように止まる (実測)。
...
まだ誰も見ていない要求を出すには `NotReviewed` を選ぶ。
...
指摘したのに中身を書き忘れた要求は、 同梱のスキルが持つ検査スクリプト `audit.sh` が
見つける (掛け方は `10-cowork-with-claude.md`)。
`review comment missing` という検査がそれである。 `Open` `Fixed` `WontFix` の
どれかなのに `REVIEW_COMMENT` が空なら発火する。

## この一式の現状

**Type**: SECTION

`03-usecases.md` の 1 件と `04-upper.md` / `06-lower.md` の 7 件が、 5 つの状態を
一通り見せている。

| UID | `REVIEW_STATUS` | 何の例か |
| --- | --- | --- |
| `UC-001` | `Open` | ユースケースへの指摘。 拒んだときの知らせ方が決まっていない |
