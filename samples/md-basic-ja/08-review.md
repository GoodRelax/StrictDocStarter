...
| 見つかるもの | どうなるか | この一式の実例 |
|---|---|---|
| **範囲が広すぎる** | 作るものが際限なく広がる。 要求まで下りてから気づくと、 書いた要求も設計もテストも捨てることになる | `UC-001` (`Fixed`)。 「別形式で受け取る」に配信や共有まで含めて読めた |
| **機能が足りない** | 筋道の途中が決まっておらず、 実装の段階で誰かが勝手に決める | `UC-002` (`Open`)。 拡張 3a が「上書きしない」で止まり、 そのあと何が起きるかが無い |

**同じ欠けが要求の側とユースケースの側の両方から見えることがある。** `UC-002` の
...
`05-upper.md` と `06-lower.md` の 7 件が、 5 つの状態を一通り見せている。
