...
**この `index.json` を直接読んではならない** — 約 210,000 tokens ある。
...
G27 の出力 127 行のうち 115 行をこの 6 つが占める (実測)。
...
4. 図を触ったなら行数を測り直す — G29。16 行以上になったら
   「図 — 15 行を超えたら別文書にする」に従って
