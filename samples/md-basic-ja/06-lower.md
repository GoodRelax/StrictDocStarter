...
SW-004 のように、 どう実現するかまで踏み込むものもある。

**上下の関係は、 常に下の側が書く。** 子のノードに `**Relations**:` を置き、 親の UID を
指す。 書き方と、 上位・下位という呼び名が親子の向きとは別物であることは
`02-guide-for-human.md` の「上位要求と下位要求」にある。 本書はその実例である。
...
本章は要求ではない。 上の 4 件がどう繋がって 1 回の実行になるかを、 図・数式・
...
図は本文に置いてある。 フローチャートには目安を置いていないので、 置き場所は
書き手が決める (「図 — 大きい図は別文書にする」)。
