...
**本書は、`ai-guide.md` の「3. 仕様書から必要な部分だけを取り出す」の詳細版である。**
主要なクエリは `ai-guide.md` に載せてある。**そちらで足りるなら本書は読まなくてよい。**
...
**文書を絞ること。** 絞らずに全体へ当てると `01-guide-for-human.md` の記法の解説に
...
一致するので、`[LINK: DOC-FIG-STATE]` で参照しているだけの `03-lower.md` は外れる。
