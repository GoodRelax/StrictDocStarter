...
本書は要求ではない。 `05-upper.md` の上位要求 3 件を **何をどう組んで満たすのか**を
1 枚の図で示す。 **要求は 1 件も足さない。**
...
| 書き出し部 | 出力先へ書く。 既にあるものは壊さない | SYS-003 |

**1 つの部が 1 つの要求に対応するとは限らない。** 受付部と変換部はどちらも SYS-001 を
担っており、 SYS-001 は 2 つの部に分かれて実現される。 逆に書き出し部は SYS-003 だけを
担う。 **要求と構造は別の軸であり、 数も対応も一致しない。**
