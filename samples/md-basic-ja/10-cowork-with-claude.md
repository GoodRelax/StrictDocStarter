...
入れたかどうかは、 `/strictdoc-md` と打って名前で呼べるかで分かる。
...
頼むときに先に決めて渡すとよいものが 3 つある。 決めずに投げると、
...
人が何を決めるか、 後で何を確かめるかの 3 つを書く。 頼み文だけを並べても、
...
**後で確かめること。** export が 2 形式とも通ること、 そして取り込んだ件数が
元の件数と合うことである。 件数は次で数える。
...
だから機械が候補を絞り、 AI が意図を判断する。 この分担がこの例の要点である。
...
下位は全要求を走査して自分を指すものを集める必要がある。 向きが違うので、
1 本のクエリで両方を出す。
...
この一式のテストケースは、 Gherkin の 3 項目を含む 8 欄で書く。
...
**後で確かめること。** export を 2 形式とも通す。 そのうえでテストが 1 つも当たって
いない要求を数え直す。
...
```text
DOC-USECASES	UC-001	Open	入力ファイルを指定形式へ変換する
DOC-UPPER	SYS-003	Open	既存ファイルの保護
DOC-LOWER	SW-003	NotReviewed	出力先の確認
```

ユースケースも `REVIEW_STATUS` を持つので、 このクエリは要求と一緒に拾う。
要求だけを見たいなら `select(._NODE_TYPE == "REQUIREMENT")` を足す。
...
大きい仕事を複数の AI に分けるときは、 ファイル単位で分け、 **同じファイルを
2 体に触らせないこと。** そしてファイルを跨ぐ約束事は先に凍結して全体へ配ること。
この一式を英訳したときに測った — 先に凍結した名前は 1 件も食い違わず、
凍結し忘れた表記だけが 4 通りに割れた。
...
行が溜まったときに読み返し、 手引きを直す材料にする。 いま 17 行ある。
