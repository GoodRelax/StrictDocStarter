...
**StrictDoc の解析には不要**である。 StrictDoc 以外の Markdown ビューアが各行を
...
1 本に繋がっている。 テストが `UC-002` を 2 度目に指しているのは、 **同じ 4 件が
...
基準でなければ、 誰もその規則を守らないから**である。 迷ったら外に出す。
...
| ファイル | バイト | `.md` 一式 (191,839 バイト) 比 |
|---|---:|---:|
| `index.json` (そのまま) | 445,727 | 2.32 |
| `jq .` を通したもの | 277,733 | 1.45 |
| `jq -c .` を通したもの | 228,320 | 1.19 |
