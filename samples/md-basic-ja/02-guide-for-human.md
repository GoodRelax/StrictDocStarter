...
管理させる道具である。 この繋がりをトレーサビリティと呼ぶ — どの要求がどこから来て、
どのテストで確かめられるかの筋のことである。 書き手は `.md` を書き、
StrictDoc は人が読む HTML と、 機械が引く JSON を出す。 導入とサーバの起動は、 この一式ではなくリポジトリ直下の
`setup-strictdoc.bat` と `launch-strictdoc.bat` が行う。 このフォルダを
`launch-strictdoc.bat` にドラッグすれば、 ブラウザでこの一式が開く。

ユースケースから上位要求・下位要求、 テストケースまでが別のファイルに分かれ、
その間が繋がっている。 書き手は、 レビューの結果を要求そのものに書く。
自分の仕様書は、 この一式を丸ごと写して中身を差し替えるところから始める。

この段落には UID が無い。 要求ではない地の文である。 StrictDoc は要求と地の文を
別のものとして扱う。 混ぜてよい。

同じ要求を `.sdoc` で書いた一式が `samples/sd-basic-ja/` にある。
2 つの記法の違いと選び方は本書の末尾にある。
...
- `03-usecases.md` — ユースケース 1 件。 利用者がどう使うかを Cockburn の形式で書く。
  上位要求はここから出てくる
- `04-upper.md` — 上位要求 3 件。 何を作るか
- `05-architecture.md` — システム構成。 何をどう組んで上位要求を満たすかの見取り図。
  要求は持たない
...
**動いているファイルを複製して書き換える。** 方法はそれだけである。 この一式が空欄のフォームでは
なく「そのまま動く文書」として配られているのは、 そのためである。

それでも止まったときは、 **原因の一覧が 1 箇所にまとまっている** —— `00-ai-guide.md` の
冒頭にある 2 つの表である。 1 つ目は**書くときに従う規則**の一覧、 2 つ目は**エラーメッセージから
引く**表で、 **壊れた後に要るのはこちら**である。 AI 向けに書いてあるが、 従う規則は同じものである。

まずエラーの 1 行目を読む。 ファイル名と行番号が出ている。
`A process in the process pool was terminated abruptly` と出た場合、 **それは本当のエラーではない。**
`--no-parallelization` を付けて export し直すと本物が出る。

中身ではなくファイルの性質の話が 2 つある。 **すべてのファイルを UTF-8 で保存すること** ——
StrictDoc はほかの文字コードを読まず、 1 枚でも違えばプロジェクト全体の export が止まり、
しかもメッセージはどのファイルか言わない。 **`.md` は LF で保存すること** —— こちらは export を
止めないので強制はされないが、 CRLF だと復帰文字がすべての欄の値に入り、 JSON export にも残って、
以後どのクエリもそれを剥がす羽目になる。 `launch-strictdoc` はサーバ起動前に両方を検査し、
見つけたものを名指しして、 直すかどうかを尋ねる。
...
その見出しは要求ノードになる。 だから書き手は、 要求ではない章に型を明示する —
でなければ文法が要求する `UID` が無いという理由で解析が落ちる。
本書の各章がそうしている。
...
StrictDoc の解析には不要である。 StrictDoc 以外の Markdown ビューアは連続する行を
1 行に繋げて表示するので、 この記号で改行させる。
...
この一式では 10 文書が同じ `basic.sgra` を読む
(`01` と `_assets/` の 2 つは宣言せず、 StrictDoc の既定の文法で通る)。
宣言しているのは 4 つのノード型である。
...
- `TEST_CASE` — テストケース。 Gherkin (振る舞いを前提・操作・結果の 3 つに割って
  書く言語。 `07-tests.md`) の `GIVEN` / `WHEN` / `THEN` と、
...
引かないものは本文に書けばよい。 `03-usecases.md` はこの基準で `UC_LEVEL` だけを
...
| 空行が 2 つ以上続く (フェンスと引用の中は除く) | `two or more consecutive empty lines are not allowed`                   |
...
`04-upper.md` の上位要求も `Relations` を持っているが、 あれは上位要求が
...
下位要求を 1 つずつ覆うと同時に、 ユースケースの主成功シナリオと 3 つの拡張を
そのままなぞっているためである。
...
書き手は出力先のパスを書かない。 StrictDoc が UID から解決する。 リンクに出る文字は
...
flowchart LR
    ...
    CHK -- "いいえ" --> STOP1["異常終了"]
    CHK -- "はい" --> EXIST{"同名の出力が既にあるか<br/>SW-003"}
    EXIST -- "ある" --> STOP2["異常終了"]
    EXIST -- "ない" --> CONV["変換して書き出す<br/>SW-001"]
    CONV --> OUT["出力ファイル"]
```
...
**目安はこれだけである — 横に並ぶものが 5 つ以上になったら、 その図を
`_assets/fig-*.md` に独立した文書として置き、 本文からは `[LINK:]` で飛ばす。**
...
これは門番ではなく目安である。 最後は書き手が決める。

上の流れ図はフローチャートなので目安を持たない。 書き手が本文に置くと決めた。
中断と後始末まで描き足した状態遷移図は別文書にした → [LINK: DOC-FIG-STATE]

横に並ぶ数で決めているのは、 それが図の幅を決めるからである。 行数は幅を決めない —
このサンプルの中でも、 8 行の流れ図が本文段の 36 % に縮み、 19 行の状態遷移図は
59 % で収まった (実測)。 ライフラインもクラスも、 1 つ増えるごとに図を横へ広げる。

行数は読む側の負担も決めない。 17 行で 114 tokens の図もあれば、 6 行で 124 tokens
の図もある (実測)。

別文書にすると、 その図は「要求を引く」作業から完全に外れる。 要求の一覧を
取り出すクエリは別文書に一切触れないため、 図の分の負担が 0 になる。 図が要るときだけ
UID で名指しして取りに行く。 ただし、 別文書にしても図そのものは大きくならない。
StrictDoc はどちらの置き方でも同じ幅の段に図を入れる (実測)。 別文書にして買えるのは、
本文が途切れないことだけである。
...
`.md` の数式は `$` と `$$` の 2 つだけである。 StrictDoc が MathJax を同梱しており、
HTML は出力フォルダの `_static/mathjax/` からそれを読む。 外部への通信は起きないので、
社内ネットワークでも切れない。
...
JSON は `out/json/index.json` に出る。 書き手は、 要求を直したら毎回出し直す。
...
出る。 あちらは記法を見比べるための最小構成であり、 ユースケース層を意図して
持たないためである。 要求だけを見れば、 どちらで書いても JSON は同じになる。
...
クエリの実例は同じフォルダの `01-ai-queries.md` に 35 本ある。 用途別に分けてあり、
出力も貼ってある。
...
| ファイル                | `.md` 一式に対する比 |
| ----------------------- | -------------------: |
| `index.json` (そのまま) |            約 2.4 倍 |
| `jq .` を通したもの     |            約 1.5 倍 |
| `jq -c .` を通したもの  |            約 1.2 倍 |

比で書いてあるのは、 バイト数が行末を CRLF にするか LF にするかで動き、 読者の手元で
再現できないためである。 比はその影響をほとんど受けない。
...
同じ要求を `.sdoc` で書いた一式が `samples/sd-basic-ja/` にある。
文法ファイルは 1 文字も違わず、 `SYS-001` から `SW-004` と `TC-001` から
`TC-004` の文面も同じである。 **あちらは記法を見比べるための最小構成であり、
本書の一式の写しではない。** あちらが持たないのは `00-ai-guide.md`・
`01-ai-queries.md`・`03-usecases.md`・`05-architecture.md`・`09-browser-guide.md`・
`10-cowork-with-claude.md` の 6 文書と、 ユースケース `UC-001` である。
逆にあちらだけが持つのは `MARKUP: Markdown` を宣言した `05-markdown.sdoc` と、
...
1. `OPTIONS: MARKUP: Markdown` が自動で付く。 StrictDoc が本文を Markdown の
   まま解釈し続けるためである。 RST に直したいなら、 書き手はその行を自分で
   消してから本文を書き換える。
...
3. `Wrong field order` に注意する。 変換した `.sdoc` を StrictDoc が読み戻す
   とき、 フィールドは決まった順序で並ぶ。 文法がその順に宣言していないと
   落ちる。
...
- 変換は `.sgra` を一緒に複製しない。 生成した `.sdoc` は `basic.sgra` を名指しするが、
  StrictDoc は出力先へその複製を作らない。 書き手が自分で複製すること
- `[LINK: UID]` を例として引用している文書が、 その引用を実際のリンクに変える。
  ...
  変換は `[LINK: DOC-FIG-STATE]` のような本物のリンクをそのまま保つ — この引用 1 箇所を
...
この一式を丸ごと写して、 中身を差し替える。 順序は次のとおりである。

1. フォルダごと複製し、 名前を自分の対象に変える
2. `03-usecases.md` から `07-tests.md` までの中身を自分のものへ差し替える。
   ファイルの数も番号も変えてよい。 番号は `00` から始める
3. `strictdoc_config.py` の `project_title` を自分の題名にする
4. `basic.sgra` はそのまま使える。 ノード型や欄を足すときだけ触る
5. `_assets/` の中身を入れ替える。 この一式の写真と図は自分の仕様書には要らない
6. `00-ai-guide.md` と `01-ai-queries.md` は残すと AI にそのまま渡せる。
   本書 (`02`) と `08` から `10` は書き方の説明なので、 消してよい
7. export を JSON と HTML の両方で通す。 通ってから中身を書き始める

**UID の付け方は最初に決めること。** 後から変えると、 関係を張り直す作業が文書の数だけ
増える。 この一式は `UC-` `SYS-` `SW-` `TC-` と層ごとに接頭辞を分けている。
