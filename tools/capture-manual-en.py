"""Re-take every screenshot in samples/md-basic-en/09-browser-guide.md.
...
disk. Point it at samples/md-basic-en itself and you will have to revert.
    ...
    cp -r samples/md-basic-en /tmp/shoot
    strictdoc server /tmp/shoot --output-path /tmp/shoot/output/strictdoc --port 5130
    python tools/capture-manual-en.py http://127.0.0.1:5130 /tmp/shots

This is the English twin of capture-manual-ja.py, and it differs from it in
four ways, each of them measured rather than guessed:

  * The Japanese tool writes the served folder into its URLs. StrictDoc serves
    documents under the name of the folder it was pointed at, so that name
    depends on what the throwaway copy is called. This tool reads the URL off
    the project index instead.
  * It takes six more shots - browser-23 through browser-28. The Japanese set
    has those six pictures, but no tool ever produced them; they were taken by
    hand and the script that took them was not kept.
  ...
  * It caps the finished images at 1600 px itself. The Japanese tool leaves
    that to the person running it.

The English the walk types lives in capture-manual-en.txt, so this file stays
English ASCII the way NFR-010 asks - the same split the Japanese tool uses.
