...
- `project_title_declined`: **新規 (FR-1168)**。 `project_title` の追記を拒否したプロジェクトを記録する map (キー = 正規化した project path、 値 = `true`)。 **`config_upgrade_declined` と分けるのは、 問いの性質が違うためである** — 世代更新は版ごとに内容が変わるので版番号で区切るが、 題名の追記はいつ聞いても同じ問いなので版で区切らず恒久的に記憶する。 利用者が手で消せば再度問われる。
