...
> **図を別文書に出す動機は「人が読みやすいこと」であって「JSON が軽くなること」ではない。** `samples/sovd-automotive-ja/` で実測したところ、Mermaid 14 ブロックは JSON 全体 576,432 バイトのうち **9,242 バイト（1.6 %）** に過ぎなかった。画像はもともと JSON に入らない。
...
| 5 | **JSON を「小さくなるもの」と考えない。** `strictdoc export --formats=json` は `json.dumps(..., indent=4)` で書くため、非 ASCII が `\uXXXX` に展開され 4 スペースで整形される。`sovd-automotive-ja` では **`.sdoc` の 2.79 倍**（トークン数）になった。JSON の利点は小ささではなく、**クエリで答えだけを取り出せること**である（[`03-sdoc-json-queries.md`](03-sdoc-json-queries.md)） |
