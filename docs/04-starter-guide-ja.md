...
| `open-strictdoc-launcher.bat`                     | **StrictDoc 自身の**デスクトップランチャー (`strictdoc launcher`。0.22.0 で追加、experimental) を起動 —— 下記「2 つのランチャー」参照。フォルダのドロップは受け取らず、ウィンドウの中で選びます。                                              | ダブルクリック               |
...
StrictDoc には**自前のランチャー**があります —— `strictdoc launcher`、**0.22.0** で追加、今も
experimental です。`open-strictdoc-launcher.bat` はそれを起動するだけのもので、コマンドを覚えて
いなくても試せます。**StrictDoc の貢献者が別に作ったもの**で、こちらのランチャーの派生では
ありません。
...
| ---------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| フォルダの開き方 | **ドラッグ&ドロップ**                                                                 | ウィンドウの中で選ぶ                                               |
| 同時に開ける文書 | **何枚でも** —— 1 文書につき 1 窓・1 ポート                                           | **1 枚だけ。** 2 枚目は「既に起動している」と断られる              |
| 起動前の検査     | 原稿が UTF-8 か検査し、CRLF の Markdown を知らせる                                    | ——                                                                 |
| ほかにできること | 配色切り替え、出力を `output\strictdoc\` に固定、`.gitignore` の案内、設定の scaffold | **Export、設定エディタ、UID 修復、`git pull` / `commit` / `push`** |
| 必要なもの       | この一式だけ                                                                          | `tkinter` の入った Python                                          |

**同梱サンプルは「同時に複数開ける」ことを前提に組んであります。** `md-basic-en` と
`md-basic-ja` を並べ、`md-` と `sd-` を並べて記法を比べる —— そのために 6 フォルダを配って
います。`launch-strictdoc.bat` を残しているのはこの一点のためです。

それ以外は StrictDoc 自身のほうが充実しており、**公式のランチャーが育つにつれて渡していく**
方針です。なお**生成物の置き場所が違います** —— こちらは `output\strictdoc\`、公式は
`output\server\` です。
