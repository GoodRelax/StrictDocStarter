...
|---|---:|---:|---:|
| `.sdoc` ソース 6 本 | 49,226 | 13,952 | 1.00 |
| JSON（`export` が出すそのまま） | 147,832 | 48,704 | **3.49** |
| JSON（`jq -c .` を一度通したもの） | 64,689 | 18,631 | **1.34** |
...
**これだけで 3.49 倍が 1.34 倍になる**（上表）。中身は同じ JSON で、クエリの結果も変わらない。**JSON をそのまま人や機械に渡す場面があるなら、export の直後に一度通しておく。**
...
| クエリ | 引数 | 出力トークン | `.sdoc` 全文 13,952 に対する比 |
|---|---|---:|---:|
| Q1 章内の要求一覧 | `--arg sec "要求も同じように書ける"` | 13 | 0.09 % |
| Q2 指定 ID の全フィールド | `SW-002` | 131 | 0.94 % |
| Q3 キーワード検索 | `--arg kw "変換"` | 59 | 0.42 % |
| Q4 上位要求（推移的） | `SW-002` | 4 | 0.03 % |
| Q5 下位要求 | `SYS-001` | 32 | 0.23 % |
