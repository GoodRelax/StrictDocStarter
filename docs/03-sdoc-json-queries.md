...
**JSON は元の `.sdoc` より小さくならない。** `samples/sd-sovd-automotive-ja/`（要求 122 件・21 文書）で測った値:

| 成果物 | バイト | トークン | `.sdoc` 比（トークン） |
|---|---:|---:|---:|
| `.sdoc` ソース 21 本 | 186,461 | 59,136 | 1.00 |
| JSON（`export` が出すそのまま） | 624,397 | 169,658 | **2.87** |
| JSON（`jq -c .` を一度通したもの） | 242,331 | 73,287 | **1.24** |
...
**これだけで 2.87 倍が 1.24 倍になる**（上表）。中身は同じ JSON で、クエリの結果も変わらない。**JSON をそのまま人や機械に渡す場面があるなら、export の直後に一度通しておく。**

> ただし本書の使い方——`jq` でクエリを当てて答えだけを取り出す——では、通しても通さなくても結果は同じである。`jq` はどちらの表記も同じに読む。**効くのは「JSON 全体を渡す」場合だけ。**

**利点は小ささではなく、答えだけを取り出せることである。** 同じ `sd-sovd-automotive-ja` に対して、本書のクエリを当てたときの出力量:

| クエリ | 引数 | 出力トークン | `.sdoc` 全文 59,136 に対する比 |
|---|---|---:|---:|
| Q1 章内の要求一覧 | `--arg sec "L1 — "` | 530 | 0.90 % |
| Q2 指定 ID の全フィールド | `AUTH-L1-001` | 208 | 0.35 % |
| Q3 キーワード検索 | `--arg kw "トークン"` | 208 | 0.35 % |
| Q4 上位要求（推移的） | `AUTH-L3-001` | 36 | 0.06 % |
| Q5 下位要求 | `AUTH-L1-001` | 93 | 0.16 % |

> **引数を表に載せたのは、値を再現できるようにするためである。** Q2 / Q4 / Q5 は同梱ファイルが `PAT-*` を埋め込んでいるので、この表の値を出すには UID を差し替えて走らせる。
>
> トークン数は tiktoken `o200k_base` による。**これは OpenAI のトークナイザであり、他のモデルのものではない。** 同じ内容の別表現どうしを比べる相対値としてのみ用いること。バイト数は実測値である。上の 2 表は strictdoc 0.27.1 / jq 1.8.1 で測り直した。
