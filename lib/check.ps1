...
function Invoke-Check {
    ...
    # FR-333: strictdoc is the tool this whole repository exists to set up, so
    # its version belongs in the diagnostic report. Probed the same way as the
    # other tools (capture-then-pick; see Get-ExistingToolVersion).
    $strictdocVer = Get-ExistingToolVersion "strictdoc"

    # Defined in lib/install.ps1, which the dispatcher dot-sources into the
    # same scope. Guarded so 'check' still produces a report if that ever
    # stops being true.
    $verifiedVer = if ($script:StrictDocVerifiedVersion) { $script:StrictDocVerifiedVersion } else { "" }
    ...
    $report = [ordered]@{
        ...
        existing_tools = [ordered]@{
            ...
            strictdoc = $strictdocVer
        }

        # FR-333: report the installed strictdoc against the version this
        # repository was verified with, so a mismatch is visible without
        # anyone having to run strictdoc by hand. No PyPI query happens here:
        # a version lookup took about 60 seconds on the reference machine and
        # 'check' is a diagnostic people run casually. 'setup-strictdoc.bat
        # upgrade' is the command that talks to the network.
        strictdoc = [ordered]@{
            installed        = $strictdocVer
            verified_version = $verifiedVer
            matches_verified = ($strictdocVer -eq $verifiedVer)
        }
    }
    ...
    # FR-333: state both versions and let the operator judge. This is not a
    # failure -- the default install pulls the newest release, so a machine
    # ahead of the verified version is the normal case, not a fault.
    if (-not $strictdocVer) {
        Write-Host "  strictdoc:     NOT FOUND (run setup-strictdoc.bat to install)"
    } elseif ($strictdocVer -eq $verifiedVer) {
        Write-Host "  strictdoc:     $strictdocVer (matches the version README records)"
    } else {
        Write-Host "  strictdoc:     $strictdocVer (README records $verifiedVer)"
        Write-Host "                 Differences are listed in docs/02-sdoc-authoring.md section 9."
        Write-Host "                 To change version: setup-strictdoc.bat upgrade"
    }
    Write-Host ""
